...
$registro_actividad_edit = NULL; // Initialize page object first

class cregistro_actividad_edit extends cregistro_actividad {
	...
	// Page object name
	var $PageObjName = 'registro_actividad_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("registro_actividadlist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->id_consulta_enfermedad->SetDbValueDef($rsnew, $this->id_consulta_enfermedad->CurrentValue, NULL, $this->id_consulta_enfermedad->ReadOnly);
...
// Create page object
if (!isset($registro_actividad_edit)) $registro_actividad_edit = new cregistro_actividad_edit();

// Page init
$registro_actividad_edit->Page_Init();

// Page main
$registro_actividad_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$registro_actividad_edit->Page_Render();
?>
...
var CurrentForm = fregistro_actividadedit = new ew_Form("fregistro_actividadedit", "edit");

// Validate form
fregistro_actividadedit.Validate = function() {
...
fregistro_actividadedit.Form_CustomValidate = 
...
fregistro_actividadedit.ValidateRequired = true;
<?php } else { ?>
fregistro_actividadedit.ValidateRequired = false; 
<?php } ?>
...
<?php $registro_actividad_edit->ShowPageHeader(); ?>
<?php
$registro_actividad_edit->ShowMessage();
?>
<form name="fregistro_actividadedit" id="fregistro_actividadedit" class="<?php echo $registro_actividad_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($registro_actividad_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $registro_actividad_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $registro_actividad->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="registro_actividad" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($registro_actividad->id->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $registro_actividad_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fregistro_actividadedit.Init();
</script>
<?php
$registro_actividad_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$registro_actividad_edit->Page_Terminate();
?>
