...
//
// Table class for direccion
//
class cdireccion extends cTable {
	var $id;
	var $parroquia;
	var $calle_principal;
	var $numero;
	...
	function __construct() {
		...
		$this->TableVar = 'direccion';
		$this->TableName = 'direccion';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "\"public\".\"direccion\"";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('direccion', 'direccion', 'x_id', 'id', '"id"', 'CAST("id" AS varchar(255))', 3, -1, FALSE, '"id"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// parroquia
		$this->parroquia = new cField('direccion', 'direccion', 'x_parroquia', 'parroquia', '"parroquia"', '"parroquia"', 200, -1, FALSE, '"parroquia"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['parroquia'] = &$this->parroquia;

		// calle_principal
		$this->calle_principal = new cField('direccion', 'direccion', 'x_calle_principal', 'calle_principal', '"calle_principal"', '"calle_principal"', 200, -1, FALSE, '"calle_principal"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['calle_principal'] = &$this->calle_principal;

		// numero
		$this->numero = new cField('direccion', 'direccion', 'x_numero', 'numero', '"numero"', 'CAST("numero" AS varchar(255))', 3, -1, FALSE, '"numero"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->numero->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['numero'] = &$this->numero;

		// descripcion
		$this->descripcion = new cField('direccion', 'direccion', 'x_descripcion', 'descripcion', '"descripcion"', '"descripcion"', 200, -1, FALSE, '"descripcion"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['descripcion'] = &$this->descripcion;

		// id_persona
		$this->id_persona = new cField('direccion', 'direccion', 'x_id_persona', 'id_persona', '"id_persona"', 'CAST("id_persona" AS varchar(255))', 3, -1, FALSE, '"id_persona"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_persona->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_persona'] = &$this->id_persona;

		// id_ciudad
		$this->id_ciudad = new cField('direccion', 'direccion', 'x_id_ciudad', 'id_ciudad', '"id_ciudad"', 'CAST("id_ciudad" AS varchar(255))', 3, -1, FALSE, '"id_ciudad"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "\"public\".\"direccion\"";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("direccionview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("direccionview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "direccionadd.php?" . $this->UrlParm($parm);
		else
			$url = "direccionadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("direccionedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("direccionadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("direcciondelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->parroquia->EditValue = $this->parroquia->CurrentValue;
		...
		$this->calle_principal->EditValue = $this->calle_principal->CurrentValue;
		...
		$this->id_ciudad->EditValue = $this->id_ciudad->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->parroquia->Exportable) $Doc->ExportCaption($this->parroquia);
					if ($this->calle_principal->Exportable) $Doc->ExportCaption($this->calle_principal);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->parroquia->Exportable) $Doc->ExportField($this->parroquia);
						if ($this->calle_principal->Exportable) $Doc->ExportField($this->calle_principal);
