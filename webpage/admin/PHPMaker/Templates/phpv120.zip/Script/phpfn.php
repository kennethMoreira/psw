...
function ew_AutoLoad($class) {
	...
	<!--## } ##-->
		...
		$file = str_replace("%cls%", substr($class, 1), "<!--##=ew_Quote(fn)##-->");
...
function ew_GetFileUploadUrl($fld, $val, $resize = FALSE, $encrypt = EW_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ew_EmptyStr($val)) {
		...
		if ($encrypt) {
			...
			$fn = "<!--##=sFnFileViewer##-->?t=" . ew_Encrypt($fld->TblName, $key) ."&fn=" . ew_Encrypt($path . $val, $key);
			if ($resize)
				$fn .= "&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} elseif ($resize) {
			$fn = "<!--##=sFnFileViewer##-->?t=" . rawurlencode($fld->TblName) . "&fn=" . ew_UrlEncodeFilePath($path . $val) .
...
<!--##include pdf-script.php/pdfclass##-->
...
class cListOptions {
	...
	function RenderEx($Part, $Pos="", $RowCnt="", $ScriptType="block", $ScriptId="", $ScriptClassName="") {
		...
		if ($ScriptId <> "") {
			...
			if ($ScriptType == "block") {
				if ($Part == "header")
					echo "<!--##=ew_Quote(ew_CustomScriptTag("%t", "oh", "%c")).replace("%t", "\" . $ScriptId . \"").replace("%c", "\" . $ScriptClassName . \"")##-->";
				else if ($Part == "body")
					echo "<!--##=ew_Quote(ew_CustomScriptTag("%t", "ob", "%c", "%i")).replace("%t", "\" . $ScriptId . \"").replace("%i", "\" . $RowCnt . \"").replace("%c", "\" . $ScriptClassName . \"")##-->";
				else if ($Part == "footer")
					echo "<!--##=ew_Quote(ew_CustomScriptTag("%t", "of", "%c")).replace("%t", "\" . $ScriptId . \"").replace("%c", "\" . $ScriptClassName . \"")##-->";
			} elseif ($ScriptType == "blocknotd") {
				if ($Part == "header")
					echo "<!--##=ew_Quote(ew_CustomScriptTag("%t", "o2h", "%c")).replace("%t", "\" . $ScriptId . \"").replace("%c", "\" . $ScriptClassName . \"")##-->";
				else if ($Part == "body")
					echo "<!--##=ew_Quote(ew_CustomScriptTag("%t", "o2b", "%c", "%i")).replace("%t", "\" . $ScriptId . \"").replace("%i", "\" . $RowCnt . \"").replace("%c", "\" . $ScriptClassName . \"")##-->";
				else if ($Part == "footer")
					echo "<!--##=ew_Quote(ew_CustomScriptTag("%t", "o2f", "%c")).replace("%t", "\" . $ScriptId . \"").replace("%c", "\" . $ScriptClassName . \"")##-->";
				echo "<span>";
			}
		} else {
	...
	<!--## if (bUserTable) { ##-->
		...
		##-->
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('<!--##=SQuote(PROJ.SecPasswdFld)##-->'), $pwd, $encrypted);
...
<!--## } ##-->
		...
		##-->
						...
						$this->setCurrentUserName(<!--##=GetFldVal(SECTABLE, sFld, FIELD.FldType)##-->); // Load user name
		<!--##
			if (bUserID) {
				FIELD = SECTABLE.Fields(DB.SecuUserIDFld);
				sFld = "$rs->fields('" + SQuote(DB.SecuUserIDFld) + "')";
		##-->
						$this->setSessionUserID(<!--##=GetFldVal(SECTABLE, sFld, FIELD.FldType)##-->); // Load User ID
		<!--##
			}
		...
		##-->
						$this->setSessionParentUserID(<!--##=GetFldVal(SECTABLE, sFld, FIELD.FldType)##-->); // Load parent User ID
		<!--##
			}
		...
		##-->
						if (is_null(<!--##=sFld##-->)) {
							...
							$this->setSessionUserLevelID(intval(<!--##=GetFldVal(SECTABLE, sFld, FIELD.FldType)##-->)); // Load User Level
						}
						$this->SetUpUserLevel();
		<!--##
			}
...
<!--## } ##-->
	...
	function SetUpUserLevelEx() {
		...
		if (count($this->UserLevelPriv) == 0 && $this->IsAdmin() && $Page != NULL && @$_SESSION[EW_SESSION_USER_LEVEL_MSG] == "") {
			...
			$Page->Page_Terminate("<!--##=sFnUserLevelTableList##-->");
		}
	...
	function CurrentUserEmail() {
		return $this->CurrentUserInfo("<!--##=sEmailFldName##-->");
	}
	<!--##
		}
	##-->
	...
	function GetUserIDByUserName($UserName) {
		...
		if (strval($UserName) <> "") {
			...
			$sSql = $UserTable->GetSQL($sFilter, '');
			if (($RsUser = $UserTableConn->Execute($sSql)) && !$RsUser->EOF) {
				$UserID = $RsUser->fields('<!--##=SQuote(sFldName)##-->');
				$RsUser->Close();
				return $UserID;
			}
		}
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('<!--##=SQuote(sFldName)##-->'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
	...
	##-->
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '<!--##=SQuote(sParentFld)##--> IN (' . $sCurUserIDList . ')';
					$sSql = $UserTable->GetSQL($sFilter, '');
					if ($RsUser = $UserTableConn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('<!--##=SQuote(sFldName)##-->'));
...
<!--## } ##-->
	...
	function UserIDList() {
		...
		$len = count($ar);
		for ($i = 0; $i < $len; $i++)
			$ar[$i] =  ew_QuotedValue($ar[$i], <!--##=sFldDataType##-->, EW_USER_TABLE_DBID);
		return implode(", ", $ar);
	}
	...
	function ParentUserIDList($userid) {
		...
		if (trim(strval($userid)) == strval(CurrentUserID())) {
			if (strval(CurrentParentUserID()) <> "")
				$result = ew_QuotedValue(CurrentParentUserID(), <!--##=sFldDataType##-->, EW_USER_TABLE_DBID);
			return $result;
		}
		...
		if (!EW_USER_ID_IS_HIERARCHICAL) {
			...
			return ew_QuotedValue(CurrentUserID(), <!--##=sFldDataType##-->, EW_USER_TABLE_DBID);
			...
			for ($i = 0; $i < $len; $i++) {
				if (strval($ar[$i]) <> trim(strval($userid))) {
					if ($result <> "")
						$result .= ", ";
					$result .= ew_QuotedValue($ar[$i], <!--##=sFldDataType##-->, EW_USER_TABLE_DBID);
				}
			}
			return $result;
	...
	<!--##~SYSTEMFUNCTIONS.GetServerScript("Global","User_PasswordExpired")##-->
...
<!--##~SYSTEMFUNCTIONS.GetServerScript("Global","AuditTrail_Inserting")##-->
