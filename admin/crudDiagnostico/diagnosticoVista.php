<?php
include_once("diagnosticoCollector.php");
$diagnosticoCollectorObj = new diagnosticoCollector();
?>
<table>
<tr><td><a href="formularioDiagnosticoInsert.php"><button> Nuevo </button></a></td></tr>
<tr>
	<th>ID|| </br></th>
	<th>Descripcion|| </br></th>
	<th>Id_registro_actividad|| </br></th>
</tr>
<?php
foreach ($diagnosticoCollectorObj->readDiagnostico() as $c){
  echo "<tr>";
  echo "<td>".$c->getid_diagnostico()."</td>";
  echo "<td>".$c->getdescripcion()."</td>";
  echo "<td>".$c->getid_registro_actividad()."</td>";
  echo "<td><a href='formularioDiagnosticoEditar.php?id=".$c->getid_diagnostico()."'> <button> Editar </button></a></td>";
  echo "<td><a href='eliminarDiagnostico.php?id=".$c->getid_diagnostico()."'>  <button> Eliminar </button></a></td>"; 
  echo "</tr>"; 
}
?>

<a href='../administracion.php'><button> volver </button> </a>
</table>

